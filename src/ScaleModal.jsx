...
import { Button, Modal, Box, Pagination, Typography } from "@mui/material";
...
const ScaleModal = ({ scaleType, keyChange, scaleData, scaleSelected }) => {
  ...
  const { setModeNotes, setScaleSelected, setScaleData, setScaleIntervals } =
    useContext(SelectedNotesContext);
  const handleOpen = () => {
    ...
    try {
      if (scaleType === "Search By Key") {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_RT_API_URL}/api/scales/tonic/${keyChange}`
        );
        ...
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_RT_API_URL}/api/scales/tonicandname/${keyChange}/${scaleType}`
        );
        if (response.data.message === "No scales to return!") {
          setSearchResultData([
            { error: "NO SCALES TO VIEW, TRY ANOTHER SEARCH SELECTION" },
          ]);
        } else {
          setScaleData(response.data.scales.sort());
          setSearchResultData(response.data.scales.sort().slice(0, 10));
  ...
  const scaleSearchByKey = async (name) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_RT_API_URL}/api/scales/name/${name}`
      );
  ...
  return (
    <div className={styles.modalController}>
      <Button onClick={handleOpen} id={styles.pickTitle}>
        2. Pick Your Scale
      </Button>
      {buttonClick &&
        (error ? (
          ...
          >
            <Box className={styles.scalesBox} id={styles.errorBox}>
            ...
            <Box className={styles.scalesBox}>
              ...
              <div className={styles.searchResults}>
                <div className={styles.searchResultsContainer}>
                  {searchResultData.map((scale) => (
                    <Button
                      style={{
                        backgroundColor:
                          scaleSelected && scale.name === scaleData[0].name
                            ? "rgb(148, 214, 148)"
                            : scale.error && "red",
                      }}
                      ...
                      variant="outlined"
                      onClick={() => {
                        scale.error ? null : scaleSearchByKey(scale.name);
                        buttonClicked();
                      }}
                    >
                ...
                <div>
                  {scaleData.length > 10 && (
                    <Pagination
                      count={Math.ceil(scaleData.length / 10)}
                      page={page}
